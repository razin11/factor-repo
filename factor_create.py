...
#df = fetch_price(ticker = "MMM", start_date = "2012-01-01", end_date = "2019-07-10")


def factor(ticker, start_date, end_date, resample_period = "Q"):
    
    
    # Uses the price dataframe and merged financial dataframe to calculate number of factors
    ...
    factor_df = price_df.merge(ttm_df, how = "outer", on = ["quarter", "ticker"])
    ...
    symbol_id = cur.fetchall()[0][0]
    
    # Calculation of factors
    # Value factors
    try:
        factor_df["ocf_yield"] = factor_df["ocf"].shift()/(factor_df["adj_close"] * factor_df["sharesos"].shift())
    except:
        factor_df["ocf_yield"] = None
    try:
        factor_df["fcf_yield"] = factor_df["fcf"].shift()/(factor_df["adj_close"] * factor_df["sharesos"].shift())
    except:
        factor_df["fcf_yield"] = None
    try:
        factor_df["adjoi_ev"] = factor_df["adjoi"].shift()/((factor_df["adj_close"] * factor_df["sharesos"].shift()) + factor_df["netdebt"].shift())
    except:
        factor_df["adjoi_ev"] = None
    try:
        factor_df["ebitda_ev"] = factor_df["ebitda"].shift()/((factor_df["adj_close"] * factor_df["sharesos"].shift()) + factor_df["netdebt"].shift())
    except:
        factor_df["ebitda_ev"] = None   
    try:
        factor_df["gaap_eps_yield"] = factor_df["gaapeps"].shift()/factor_df["adj_close"]
    except:
        factor_df["gaap_eps_yield"] = None
    try:
        factor_df["b_p"] = factor_df["equity"].shift()/(factor_df["adj_close"] * factor_df["sharesos"].shift())
    except:
        factor_df["b_p"] = None
    try:
        factor_df["div_yield"] = factor_df["dps"].shift()/factor_df["adj_close"]
    except:
        factor_df["div_yield"] = None
    try:
        factor_df["shareholder_yield"] = ((factor_df["dps"].shift() * factor_df["sharesos"].shift()) + factor_df["buyback"].shift())/(factor_df["adj_close"] * factor_df["sharesos"].shift())
    except:
        factor_df["shareholder_yield"] = None
    
    # Quality factors
    try:
        factor_df["roa"] = factor_df["adjoi"].shift()/factor_df["assets"].shift()
    except:
        factor_df["roa"] = None
    try:
        factor_df["roic"] = factor_df["adjoi"].shift()/(factor_df["equity"].shift() + factor_df["ltdebt"].shift() + factor_df["stdebt"].shift())
    except:
        factor_df["roic"] = None
    try:
        factor_df["netdebt_ebitda"] = (factor_df["ltdebt"].shift() + factor_df["stdebt"].shift() - factor_df["cash"].shift())/factor_df["ebitda"].shift()
    except:
        factor_df["netdebt_ebitda"] = None
    try:
        factor_df["fcf_conversion"] = (factor_df["fcf"].shift()/factor_df["sharesos"].shift())/factor_df["gaapeps"].shift()
    except:
        factor_df["fcf_conversion"] = None
    try:
        factor_df["ebitda_margin"] = factor_df["ebitda"].shift()/factor_df["sales"].shift()
        factor_df["ebitda_margin_exp"] = (factor_df["ebitda_margin"].shift()).diff()
    except:
    ...
    # Growth factors
    try:
        factor_df["sales_growth"] = (factor_df["sales"].shift()/factor_df["sharesos"].shift()).pct_change(1)
    except:
        factor_df["sales_growth"] = None
    try:
        factor_df["fcf_growth"] = (factor_df["fcf"].shift()/factor_df["sharesos"].shift()).pct_change(1)
    except:
        factor_df["fcf_growth"] = None    
    try:
        factor_df["gaapeps_growth"] = (factor_df["gaapeps"].shift()).pct_change(1)
    except:
        factor_df["gaapeps_growth"] = None
    
    factor_df["symbol_id"] = [symbol_id for i in range(len(factor_df))]

    return factor_df


#df = factor(ticker = "MMM", start_date = "2012-01-01", end_date = "2019-07-10", resample_period = "Q")
#df
#df.to_csv("test7.csv")
